...
def load_documents():
    ...
    if not documents:
        st.error("Nenhum documento foi carregado. Verifique o caminho dos dados e o padrão do arquivo.")
    else:
...
def split_text(documents: list[Document]):
    ...
    if not chunks:
        st.error("Nenhum chunk foi criado. Verifique o processo de divisão de texto.")
    else:
...
def save_to_chroma(chunks: list[Document]):
    ...
    db.persist()
    st.info("Base de dados Chroma criada e persistida com sucesso.")
...
def main():
    ...
    try:
        ...
        st.error(f"Erro ao conectar com o banco de dados Chroma: {str(e)}")
        return
    ...
    if st.button("Buscar"):
        ...
        if len(results) == 0 or results[0][1] < 0.: # Critério de aceitação mínima de resposta
            st.warning(f"O banco de dados é: {db}")
            st.warning(f'Os resultados foram: {results}')
            st.warning(f"Não foi possível encontrar resultados; pois {len(results)}")
        else:
